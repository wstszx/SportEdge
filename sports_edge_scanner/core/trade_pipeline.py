...
from sports_edge_scanner.models import CandidateOrder, OrderBook, RiskDecision, ShadowOrder


AcceptedOrderHandler = Callable[
    [CandidateOrder, RiskDecision, OrderBook, int, datetime],
    dict[str, Any],
]
...
def _order_id(run_id: str, suffix: str, index: int) -> str:
    return f"{run_id}-{suffix}-{index}"
...
def run_trade_scan(
    *,
    ...
    run_id: str,
    order_id_suffix: str,
    on_accepted_order: AcceptedOrderHandler,
    ...
    for market in markets:
        ...
        for candidate in candidates:
            ...
            projected_order = ShadowOrder(
                order_id=_order_id(run_id, order_id_suffix, order_index + 1),
            ...
            order_index += 1
            handler_result = on_accepted_order(
                candidate,
                decision,
                book,
                order_index,
            ...
            for key, value in (handler_result.get("counts") or {}).items():
                extra_counts[key] = extra_counts.get(key, 0) + int(value)
            if not handler_result.get("accepted", True):
                continue
